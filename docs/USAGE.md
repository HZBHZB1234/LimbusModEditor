...
3. **自动扫描**：新建/打开的空项目会自动弹出「扫描游戏资源」窗口：
   ...
   已扫描条目走程序目录索引缓存（`cache/unity-cache-index.json`），
   再次扫描接近瞬时。个别损坏/未知 bundle 会作为逐条诊断报告，不中断整体。
4. **编辑**：扫描完成后在列表中搜索（类型/状态/大小/Path ID 筛选），
   选中资源即可替换 PNG、编辑 Sprite 元数据或 Unity 字段。首次编辑某个
   缓存 bundle 时编辑器自动把它复制进项目 `sources/cache/`（每 bundle
   一次），此后缓存被游戏清理/换键也不影响你的项目。
