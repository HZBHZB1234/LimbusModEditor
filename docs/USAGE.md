...
- **右键菜单**：资源列表右键 = 替换… / Unity 字段编辑 / 十六进制预览 /
  撤销此资源的修改 / 复制资源路径；右键会先选中所在行。
...
- **长操作反馈**：扫描窗口显示预计剩余时间；一键导出有进度窗口
  ...
  「打开模组目录」「打开项目文件夹」快捷按钮。
- **列表友好化**：大小列显示 KB/MB/GB；已修改行以金色文字高亮；
  悬停行显示完整资源路径；右栏「已修改资源」按资源数统计
  （同一资源重复替换只算一次）。

## 1.3 工作台标签页与目录树（P3.8）

- **活动栏**：主窗口最左侧一列图标（📦资源 / 📝文本 / 🧩静态 /
  🗂模组 / 📖教程 / ⚙设置）。点击图标打开对应工作台标签页；已打开的
  标签再次点击 = 直接激活，不会重复创建。
- **标签页**：「资源工作台」固定在第一位；文本模组、静态数据模组、
  模组管理以标签页形式打开（不再弹独立窗口），标题带 ✕ 可关闭，
  关闭后自动激活相邻标签。`Ctrl+Tab` / `Ctrl+Shift+Tab` 循环切换。
- **目录树浏览**：资源工作台中部有「☰ 列表 / 🗂 目录树」切换。目录树
  把资源按容器路径像文件夹一样逐层展开（目录显示「名称 (N)」）：
