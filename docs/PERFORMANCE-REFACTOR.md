...
2026-09-19 的检索层、缓存内存和音频读取优化及实测结果见 [本轮性能报告](PERFORMANCE-2026-09-19.md)。
...
- `Locate(logicalPath)` 给的是**全局名次**。只有**真的什么条件都不加**的视图里它才等于页下标 ——
  资源页默认勾选「仅容器内资源」（`ContainerOnlyFilter`），并且默认**隐藏静态数据表**
  （`ShowStaticTables = false`），所以「默认视图」已经不是无筛选了。「无筛选」现在必须显式写成
...
**默认视图必须把它们藏掉、勾上「显示静态数据表」必须一条不少地回来**，两个方向都在查询矩阵里，
所以这三个维度同时压「误藏」和「误放」；类型覆盖 class-id 映射 / ref-type 伪 id /
映射不到保留类型树结论、**不同 bundle 显示路径故意重名**以压住打平兜底），
...
而这 5 万条里有 48,575 条只是用来「筛掉 2,801 条静态数据」的（2026-09-15 之后那 2,801 条
已经由索引在 SQL 侧排掉，所以现在是 142 MiB —— 见「静态数据表筛选回到默认视图」）。
...
## S3b：列表 / 页码条 / 目录树改走 catalog（2026-09-15）

S3a 只做出了门面，没人用。这一步把资源页的三个消费者换过去。

### 列表：一页就是列表里的全部行

`RunSearchAsync` 不再 `project.Assets.ToArray()`（127 万引用）也不再构造 5 万条
`AssetRow`，改成 `catalog.Page(query, pageIndex * pageSize, pageSize)`：命中集只以
**来源标记**（8 字节）的形式参与排序，记录只物化**当前这一页**。

新增页码条（首页 / 上一页 / 页号 / 总页数 / 下一页 / 末页 + 每页条数 + 共 N 条）。
它只服务列表视图 —— 目录树是惰性分层的，没有「第几页」这回事。页码越界（筛选变化后
原来那一页已经不存在）会被夹到最后一页，而不是给一片空白。

**精确跳转**（`Reveal`）的语义跟着变了：以前是「按关键词过滤，等结果挂上列表后在其中
找那一行」，现在是「`IndexIn` 算出它在命中集里第几条 → 翻到它所在的页 → 选中」。
所以目标落在第 200 页也能直接翻过去，而不是要求用户自己去翻。

### 目录树：节点只记一个区间

`AssetCatalogTree`（`Application/Catalog/AssetCatalogTree.cs`）的每个节点只记
**命中序列里的一个区间** `[Start, End)`，展开时才把这段区间按下一层路径段切开。
能这么做的前提是：目录树的分组序（显示路径自然序）**就是** `catalog_rank` 的序，
所以「同一目录下的资源」在序列里必然连续；而同层的资源叶子又必然排在同名目录组之前
（`"A/X"` 比 `"A/X/y"` 短，自然序在前），于是叶子不会把任何目录组切断。

口径只有一份：显示路径走 `AssetDisplay.TreePath` / `SegmentAt`，重名消歧直接调
`AssetTreeNode.DistinguishLeaves`（与旧的记录树**同一个函数**），同层排序用
`AssetDisplay.ComparerInstance`。`AssetTreeBuilder` 因此降级为**参考实现**
（生产路径不再调它，但测试拿它逐节点对账）。

### 项目态补充集：列表/树都可能「索引里没有」的资源

索引行只覆盖扫描到的缓存引用，而项目会自己长资源：音频工作台「提取样本到项目」、
`ModImportService` 导入的旧式模组（Carra / Lunartique / Rebank）都是往
`ModProject.Assets` 里加一条索引里不存在的记录。**只查索引会让这些资源从列表里消失**，
所以 `IAssetStateSource` 多了 `ProjectOnly`（与「路径 → 记录」的字典同一趟算出来，
不多付一次百万级遍历），命中集由「索引名次 + 补充集下标」两种来源混排。

代价：**有补充集时「按名称」也必须算排序键**（旧路径靠名次序免费拿到名称序）。
补充集为空时保持原快路径不变。

### 真实规模实测（`logs/run_catalog_probe.py`，1,275,623 行）

| 步骤 | 耗时 | 分配 |
|---|---|---|
| `ReadCandidateRanks`（默认视图，仅容器内） | 915 ms | 0.5 MiB |
| `Page(默认视图, 0, 200)` 首页 | 1,294 ms | 151.6 MiB |
| `Page(默认视图, 51,176, 200)` **末页** | 1,070 ms | 150.3 MiB |
| `new AssetCatalogTree`（取命中序列 51,376 条） | 1,025 ms | 150.1 MiB |
| `Roots()` 根层 | 579 ms | 168.0 MiB |
| 展开最大根「Assets」 | 511 ms | 168.7 MiB |
| 再展开「Resources_moved」（47,590 条） | 465 ms | 155.4 MiB |
| **对照面**：物化全部 51,376 条 + 旧 `BuildRoots` | 679 ms | 51,376 条**常驻** 108.3 MiB 堆 |

三件事值得记下来：

1. **首末页同价（1,294 vs 1,070 ms）** —— 「与页深无关」这条成立，旧实现末页 12.2 s。
2. **每取一页仍要 150 MiB 分配** —— 瓶颈还是 S3a 记下的那条：判据
   `Matches` 只接受 `AssetRecord`，所以候选集 5 万条要**逐条造记录再复核**，只是
   造完就丢、不再常驻（对照面那 679 ms 带走的是 108 MiB **常驻**堆）。
   真正落地「只物化一页」要把判据下沉到行级（见上一节末）。
3. **目录树每层都是重新物化一次那段区间**，不再共享内存（对照面建一次树之后
   深层展开免费）。换来的是树的常驻成本从「命中集条数 × 1,028 字节」降到
   8 字节/条（51,376 条 ≈ 0.4 MiB），且与展开到多深无关。

### 守等价性的测试

`tests/LimbusModEditor.Domain.Tests/AssetCatalogTreeTests.cs`：**同一份数据当场建两棵树
逐节点对账** —— 名称、深度、叶子判定、资源数、展开键（`Path`）、叶子指向的资源，
递归到叶。数据集铺满树的形状维度：三层嵌套、同目录重名叶子、无容器条目、根级单段路径、
以及「一条路径同时是叶子又是目录前缀」。写这条用例时当场抓到过一处实现错误
（叶子判定的段数条件差一，`depth + 1` 写成了 `depth + 2`），根层节点数 6 ≠ 9。

`AssetCatalogTests` 另加一条：项目态补充集参与分页、排序、定位，且**不总是被甩到末尾**。

## 静态数据表筛选回到默认视图（2026-09-15 · 改走扫描期落库）

「资源工作台默认隐藏静态数据表」在 `f0cd363` 被整体删掉（当时的判断是这条筛选不重要），
随后用户改主意：**筛选要保留，但必须按「扫描时分析」做**。老实现的病根从来不是「有这条筛选」，
而是**判据在运行时逐行重算**：每查一次列表，对有容器条目的 5 万行各跑三道字符串判据
（真实库 1.8 s/次）；而且两处判据来源不同（列表重算 / 预览读元数据标记）⇒ catalog 换键后
「时灵时不灵」，这正是用户最初报障的那条。

做法：结论变成 `assets.static_kind` 这一列**位掩码整数**（`AssetStaticClassifier`：位1 catalog
内层键命中 / 位2 bundle 名前缀 / 位4 容器路径前缀），**扫描期算完落库**；查询期只做一次
`static_kind = 0` 的整数比较，与 `Matches` 末条判据是**同一个结论**（所以 SQL 侧不留超集余量）。

| 环节（真实库副本 1,275,623 行） | 数字 |
|---|---|
| 老库打开时的一次性就地迁移 | **3,242 ms**（ALTER 加列 + 就地回填 + 换 `ix_assets_named` 列序 + 建 `ix_assets_static`）；**迁过的库再打开 701 ms**（迁移不重复执行） |
| └ 派生层是否被重建 | **否**（`EnsureDerived().Rebuilt = false`，`derived_state` 仍是 `d2`）⇒ 不重扫 1,471 个 bundle |
| 静态行数（位掩码并集） | **2,802** = 位4 的 2,801 ∪ 位1 的 1,393（重叠 1,392）；位2 = **0**（缓存目录名是裸哈希，前缀判据天然命中不了） |
| 默认视图（仅容器内）命中数 | 51,376 → **48,575**（藏掉 **2,801**，与老功能当年藏掉的是同一批） |
| `Page(默认视图, 0, 200)` | **1,492 ms / 142.0 MiB**；同一份数据**显示**静态表是 1,684 ms / 150.6 MiB |
| 末页 `Page(默认视图, 48,375, 200)` | 1,417 ms（「与页深无关」仍然成立） |

也就是说：**这条筛选不是「花时间换干净」，它顺手更快** —— 少 2,801 条候选就少 8.6 MiB 分配、
少 192 ms，因为候选集的开销全在「逐条造记录 + 复核」上（见上节「仍存在的开销」）。

**列序与谓词是量出来的，不是推的**（`logs/probe_static_index_shape.py`）：`ix_assets_named`
以 `static_kind` **打头**，并且「有容器条目」的谓词从 `trim(coalesce(container_entry,'')) <> ''`
换成 `IS NOT NULL AND container_entry <> ''` —— 后者才与部分索引的谓词**同形**，planner
才用得上；SQL 侧实测 1,844.7 → **157 ms**。空白串/带前后空格的行在真库里是 **0** 条，
而这个换法的差异方向是「多带行」（SQL 超集、内存判据复核掉），不是漏行。

**两个洞必须一起堵**（否则结论会静默过期，症状就是「时灵时不灵」）：
① `PersistAll` 只 upsert **变化过的** bundle（新鲜度看 `__data` 的 size/mtime）⇒ catalog 换了键
但文件没变时，静态结论不会刷新 —— 所以它对「没重写但结论变了」的 bundle 单独做一次
**位级刷新**（只替换 bundle 级两位，行级的容器路径位原样保留，且**不**清 `derived_state`）；
② `AssetCatalog.MergeProjectState` 用项目元数据**整体覆盖**索引事实 ⇒ 会覆盖静态结论，
所以静态键是唯一的例外：**索引才是权威**（`.lmeproj` 可能是几个月前存的，里面写着老形态的
`"true"`）。

**注意「无筛选」已经不存在了**：默认视图现在带静态隐藏，要「全局名次 = 页下标」必须显式写
`ShowStaticTables = true`。而取消「仅容器内」的全量视图仍是 **42.6 s/页**（1,272,821 条候选
全都要造记录，3.7 GiB 分配）—— 那与静态筛选无关，账记在 S3c / 「判据下沉到行级」上。

复现：探针工程 `%TEMP%\lme-catalog-probe`（`logs/run_catalog_probe.py` 负责暂存真实库副本并
驱动它；新增 `--quick` 跳过 42 s 那一档）、`logs/probe_static_index_shape.py`（列序/谓词选型）、
本次验收原始输出 `logs/static-acceptance.out.txt`。临时工程被清理时按本节表格重建即可 ——
全程只用 `AssetCatalog` / `UnityCacheSqliteIndexStore` 的公开 API。

## 仍存在的开销

**S3b 之后，资源页的常驻成本已经不再是「全量 AssetRecord」**：列表只留一页（≤ 页大小条）、
目录树只留命中序列（8 字节/条）。剩下的大头是**每次查询都要把候选集逐条造记录再跑判据**
（默认视图 5 万条 ≈ 1.3 s / 150 MiB 瞬时分配），以及 `ModProject.Assets` 本身仍是全量
引用集合（约 1,250.9 MiB）。磁盘代价见上一节：派生列 + 稠密名次约 +217 MiB（`d2`，含
`lp` 检索）+ 16.8 MiB（反查索引）。
**S3a 做出了查询门面并证明与旧搜索逐行等价，S3b 让列表/页码条/目录树改走它**（取页
5,671 → 1,294 ms、末页 12.2 s → 1,070 ms）。剩下的工作是：把 `ModProject.Assets` 收窄成
「项目态记录」并迁移其余用法（S3c：`AssetCatalog.Resolve` 已经能替代「从 Assets 里按
LogicalPath 捞记录」，是迁移的落点）；以及把判据下沉到行级，让候选集不必逐条造记录
（那个改完预计把 1.3 s 压到 ≈ 0.3 s / 页）。
其他四个工作台/派生缓存没有改表布局。
XZ 压缩与 Unity 重打包仍由原库处理；本次不改变加载器格式或扩展未知 Unity 格式的写回支持。

## S3c 迁移面（`ModProject.Assets` 收窄成「项目态记录」）

先量过再动手（2026-09-15）：`project.Assets` 全仓库用法 **194 处 / 54 文件**（含测试），
按用法分类：

| 用法 | 处数 | 备注 |
|---|---|---|
| `Count` | 61 | 多数是「有没有东西」的空判断，可换成索引侧 `Count` |
| 全量遍历 | 42 | 真正的障碍，需逐处确认是否只关心已编辑子集 |
| 写入 | 60 | 新增/更新项目态记录，本身与收窄一致 |
| `ByIdentity` | 11 | 按 `LogicalPath` 查，落点 = `AssetCatalog.Resolve` |
| `HasEdits` | 9 | 只读 `IsEditedCacheAsset`，与收窄相容 |
| XAML/绑定 | 2 | 见下「陷阱」 |

- 6 处 `FirstOrDefault(x => x.AssetId == …)` 是 **O(127 万) 线性扫描**。**调用点本来就
  持有对象**，所以改签名即可 —— **不要为此建字典索引**（那是又一份全量内存）。
- 全仓库**没有** `Remove`/`Clear`、**没有** XAML 绑定、**没有** `CollectionChanged` 订阅
  ⇒ 收窄成「项目态记录」不会破坏增量通知语义。
- **两个必须提前处理的陷阱**：
  1. `SpinePreviewService.Index()` 用「集合**引用** + `Count`」当缓存键 ⇒ 改成按页查询后
     这个键**必然失效**（引用变了/条数变了），必须换成本身的语义键。
  2. `SkipReferenceAssetsConverter` 是按 `ObservableCollection<AssetRecord>` **类型**注册的
     ⇒ `Assets` 属性类型一改（换集合类型），该 converter 会**静默不再生效**，项目文件
     又从 KB 级涨回 GB 级，而且**不报错**。
