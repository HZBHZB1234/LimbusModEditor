...
3. 目录配置是无感自动化的：**打开/新建项目时编辑器会自动获取缺失的目录**
   （游戏目录、Unity 缓存、模组目录），左栏「目录状态」实时显示 ✓/⚠/✗；
   手动设置过的值永远不会被自动覆盖，事后补齐可点左栏 **「自动获取目录」**，
   精细调整全部集中在 **「项目设置…」** 二级窗口：
   - **游戏目录** — `LimbusCompany.exe` 所在目录（应用并启动调试时需要）。
     自动获取会扫描已知的 Steam 库（注册表 Steam 路径 + `libraryfolders.vdf` +
     常见安装位置），找到包含 `LimbusCompany.exe` 的安装目录并自动填入；
     未找到时请在项目设置中手动选择。
   - **Unity 缓存目录** — 游戏的 Addressables 缓存根目录，用于导入 bundle 时补全
     依赖信息。真实布局为 `LocalLow/Unity/ProjectMoon_LimbusCompany`（可能是迁移到
     其它盘的 junction）下的 `<32位hex外层键>/<32位hex内层键>/__data`。自动获取只
     采用验证过实际包含缓存条目的候选目录；在项目设置里手动获取时，若候选多于
     一个会列出条目数量供你确认。
   - **模组目录** — 你的模组存放根目录（导出时默认位置）。真实加载器默认使用
     `%APPDATA%\LimbusCompanyMods`，自动获取直接采用；配置后可用 **「管理已安装
     模组」** 查看已装模组并按加载器自身的 `_disable` 后缀约定切换启用/禁用
     （只重命名，不改文件内容；目录缺失时会先尝试自动获取）。
   - **FMOD DLL 目录** — 含 `fmod64.dll` / `fsbank64.dll` 的目录。**必须使用你合法获得的
     DLL**，本工具不附带任何 FMOD 二进制文件；没有它时音频只能探测索引，不能解码。
     该目录从不自动获取；在项目设置中配置后可用 **「检测兼容性」** 在不加载任何
     DLL 的前提下核对位数、版本与导出符号是否齐全。
...
- 完整 FSB5 原样替换不需要 DLL。
- **「检测兼容性」（项目设置窗口内，P2.2）**：无需加载 DLL 即可查看目录中
  fmod64.dll /
