...
| 报障 | 根因（一句话） | 现在的口径 |
|---|---|---|
| 资源工作台仍含 static-data | 静态标记只由 catalog 判定产生，`catalog` 缺失/滞后时标记为假 | **2026-09-15 按用户决定整体移除该筛选**：资源列表不再按静态数据过滤，静态数据表在资源工作台里正常可见、可编辑。曾走「元数据标记 + bundle 名兜底 + 容器路径前缀」三道判据（见 6.2.1）；标记本身仍由扫描写入，现仅供预览通道与静态工作台使用 |
