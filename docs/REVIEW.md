...
见仓库提交记录中的复审提交；发现的问题按严重级别修复或归档为 ROADMAP 后续项。
...
1. **自动定位 Unity 缓存目录**（类比游戏目录定位）：从游戏目录推导
   `LimbusCompany_Data/StreamingAssets` 与注册表/常见 LowData 路径，
   减少第二项手动配置。
2. **导出向导内联预校验**：向导点击"开始导出"前先跑 Unity 字段预校验与
   引用完整性 dry-run，把失败原因直接显示在矩阵下方，避免导出中途失败。
3. **批量替换向导**：选择一个文件夹，按文件名匹配逻辑路径批量登记替换
   （现在必须逐资源选择）；与筛选行配合（"仅未替换"）形成完整工作流。
4. **Sprite 图集一键拆分登记**：拆分图集后自动为每个子图登记 Sprite 资源与
   替换文件，替代当前的手动逐个替换。
