...
- **列表友好化**：资源视图默认显示「名称 / 类型 / 状态 / 大小」四列，
  ...
  资源数统计（同一资源重复替换只算一次）。
- **三种预览**（右栏「预览」区，随选中项自动切换）：
  - 图像（Texture/Sprite）：解码缩略图 + 尺寸/格式/Alpha 说明；
  - 文本（Text/JSON 或 .txt/.json/.csv 等）：直接显示可读正文（UTF-8 /
    UTF-16 自动识别，超过 4000 字只显示开头）；非 UTF 文本或二进制负载会
    明确说明并指向「十六进制预览」；
  - 音频（Bank 音频）：有 FMOD DLL 时给「▶ 试听」（解码成 WAV 用本机播放，
    不写入项目，切换选中项/关窗自动停止并清理临时文件）；缺 DLL 或 Unity
    音频对象时说明原因，不假装能播。
