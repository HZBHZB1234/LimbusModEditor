...
1. **导出模组**：按来源格式或选择 Carra/Carra2/Rebank/Lunartique 输出。Lunartique
   安装包会通过对象载荷哈希对比转换成对象级输出；不支持的删除型资源会被明确报告，
   而不是盲目复制。
2. **构建调试覆盖层**：在 `builds/` 生成应用文件。
3. **应用并启动调试**：先备份游戏目录中被覆盖的文件到 `backups/`，再应用覆盖层并
...
| 音频导出灰显 | 未配置 FMOD DLL 目录，或目录里缺少 fmod64.dll/fsbank64.dll |
...
Bank 探测、图像/图集操作、覆盖层备份/恢复、Unity 字段树与编辑校验，以及 P1.2 的
依赖解析、引用者扫描、指针语义校验和重写后引用完整性验证。
