...
3. **多格式槽位的产物命名（本轮修订）**：`_fmod/bank/` 与 `_fmod/rebank/` 按<b>目标 bank 文件名</b>命名
   （加载器按文件名匹配目标 bank，别的名字一律无效）；`_data/carra/`、`_data/lunartique/`、
   `_static/staticmod/` 是**单文件归档**，按**来源名**命名——`<来源文件名>.carra` / `.zip` / `.staticmod`
   （本机真实数据里 `环指回调.staticmod`、`测试-李箱浮士德攻击容量修改.staticmod` 就是这个惯例），
   项目里没有该来源（纯新建项目）时退回 `<项目名>`。理由：用户在一个目录里可能存在多个模组，
   全部叫 `<项目名>.*` 会互相覆盖，而按来源命名既保持习惯又能并存。
   ⚠ 副作用：同目录若已存在同名归档会被覆盖 —— 导出前对同名产物做<b>覆盖前提示/备份</b>（实施于 S4）。
...
### 3.4 `bus` 通道的**可表达性门**（S5 实测修订，必须与实现一致）

写 bus 规则集前必须先检查「这条 diff 能不能用 bus 的 `set` 语义无损表达」，否则会产出
**加载器不报错但改动丢失**的规则集（对制作者最坏的结果）。逐条判据（依据 `webutils/fancy/bus.py`
的 `_PathResolver` 与 `_apply_replacements`）：

| diff 里的操作 | bus 能否表达 | 处理 |
|---|---|---|
| `replace /a/b/c`（值合法） | ✅ `path="a.b.c"` + `replacements[{set:新值}]` | 写 rule |
| `add` 到**原文档已存在**的末段 | ✅ 同上 | 写 rule |
| `add` 到原文档**不存在**的位置（对象新键、数组新增元素） | ❌ 赋值格式改了会被加载器**静默跳过**（bus 的 `_resolve_paths` 返回空；pathset 的 jsonpatch `add` 下标越界失败） | **整份不产出** |
| `remove` | ❌ 没有删除动作 | **整份不产出** |
| 数组元素增删（diff 里表现为 `remove`+`add` 序列） | ❌ 数组长度变了 | **整份不产出** |
| 路径段不是 `A-Za-z_` 标识符（如含 `-`） | ❌ 加载器按 `.` 分段解析 | **整份不产出** |
| 数组下标路径（原文档已存在该下标） | ✅ `path="list[3].name"` | 写 rule |

**「整份不产出」而不是「只写能写的部分」**（S5 实测确定）：半份规则集/清单会让加载器只改一部分，
而用户以为全改了 —— 这比「没有这个格式」危险得多。因此只要**有一个**操作表达不了，
该格式就整份跳过，并在诊断里列出每个表达不了的操作与原因；`patch`（RFC6902）不受影响，
它表达得了全部操作，报告里会指明「完整改动见 patch 格式」。
